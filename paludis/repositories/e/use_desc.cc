            ...
            if (f.stat().is_regular_file_or_symlink_to_regular_file())
            {
                        ...
                        line != line_end ; ++line)
                {
                    ...
                    if (std::string::npos == q)
                        global_descs.insert(std::make_pair(std::make_pair(prefix, lhs), rhs));
                    else
                        local_descs[QualifiedPackageName(lhs.substr(0, q))].insert(std::make_pair(std::make_pair(prefix, lhs.substr(q + 1)), rhs));
                }
            }
        }
