        ...
        void add(const FSPath & f, const ChoicePrefixName & prefix)
        {
            if (f.stat().is_regular_file_or_symlink_to_regular_file())
            {
                        ...
                        line != line_end ; ++line)
                {
                    ...
                    {
                        auto pair = std::make_pair(ChoicePrefixName(prefix), UnprefixedChoiceName(lhs));
                        global_descs.insert(std::make_pair(pair, rhs));
                    }
                    else
                    {
                        auto pair = std::make_pair(ChoicePrefixName(prefix), UnprefixedChoiceName(lhs.substr(q + 1)));
                        local_descs[QualifiedPackageName(lhs.substr(0, q))].insert(std::make_pair(pair, rhs));
                    }
                }
            }
        }
